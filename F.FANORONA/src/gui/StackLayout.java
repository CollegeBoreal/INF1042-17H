 ...
 * horizontal stacks (both using StackLayout). In that case, it would usually be
 * good to add the horizontal panels using the tag "Wide Flush", so that spacing
 * comes out evenly.
 * <p>
 * 
...
public class StackLayout implements LayoutManager {
	...
	int margin = 2;
	Hashtable<Component, int[]> codeTable = new Hashtable<Component, int[]>();
	...
	public void layoutContainer(Container parent) {
		...
		int W = sz.width - in.left - in.right, H = sz.height - in.top
				- in.bottom;
		...
		int rubber = ((nFills != 0) || (nRubber == 0) ? 0 : Math.max(0,
				(L - sum) / nRubber)), fill = (nFills == 0 ? 0 : Math.max(0,
				(L - sum) / nFills));
